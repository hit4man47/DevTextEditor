...
public class MainActivity extends AppCompatActivity
{
    ...
    public static long endIndex=0;
    public static File myFile;

    private final long CACHE_CAPACITY=512 ;//BYTES
    private boolean ignoreScroll;

    private void initialiseMyApp(Bundle savedInstanceState)
    {
        ...
        final TypedArray typedArray =
                obtainStyledAttributes(R.styleable.Dev_Text_Editor);

        if (typedArray.hasValue(R.styleable.Dev_Text_Editor_BackgroundColour))
            textView
                    .setBackgroundColor(typedArray
                            .getColor(R.styleable
                                    .Dev_Text_Editor_BackgroundColour, 0));
        typedArray.recycle();
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        initialiseMyApp(savedInstanceState);
               ...
               if(scrollView!=null)
                {
                    ...
                    b=scrollView.getHeight();
                    c=scrollView.getScrollY();
                    if(a==(b+c) && c>0)
                    {
                        //Toast.makeText(MainActivity.this, "end", Toast.LENGTH_SHORT).show();
                        Log.e("Scroller:","at end");
                        Log.e("Scroller:",a+"***"+b+"***"+c);

                        if(!edit && endIndex<myFile.length())
                        {
                            //MORE BYTES TO READ AT END
                            Log.e("changing",startIndex+"+++");
                            startIndex=startIndex+(long)(0.5*CACHE_CAPACITY);
                            Log.e("changed",startIndex+"@@@@@@@");
                            ReadTask read = new ReadTask(false);
                            read.execute();
                        }
    ...
    private void readFile(Uri uri)
    {
        ...
        {
            //file does not exist
            textView.setText(R.string.fileNotExist);
        }
        else
        {
            if (!myFile.canRead())
            {
                //file cannot be read
                textView.setText(R.string.fileNotRead);
            }
            else
            {
                ...
                startIndex=0;
                ReadTask read = new ReadTask(true);
                read.execute();
            }
        }
        dirty = false;
    ...
    {
        String path = HelpingBot.getPath(this, uri);
    ...
    {
        private final boolean initialRead;
        private ReadTask(boolean initialRead)
        {
        ...
        protected String doInBackground(Void... params)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    fileInputStream.getChannel().position(startIndex).position();
                }
                catch (Exception e)
                {
                    return null;
                ...
                try
                {
                    while ((byteValue = fileInputStream.read()) > 0)
                    {
                        totalBytesRead++;
                        if(totalBytesRead>=CACHE_CAPACITY)//5KB
                        {
                            break;
                        }
                ...
                endIndex=startIndex+totalBytesRead-1;
        ...
        {
            Log.e("PostRead:",startIndex+"##"+endIndex);
            if(result==null)
            {
            ...
            {
                //textView.setTextKeepState("12121213");
                textView.setTextKeepState(result);
                //textView.append();
                //Log.e("&&&",(scrollView.getBottom()/2)+"==");
                //textView.setScrollY(scrollView.getBottom()/2);
            }
